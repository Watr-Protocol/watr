...
use pallet_evm::{
	Account as EVMAccount, EVMCurrencyAdapter, EnsureAddressRoot, EnsureAddressTruncated,
...
pub const MICRO_WATR: Balance = 1_000_000_000_000;
pub const MILLI_WATR: Balance = 1_000 * MICRO_WATR;
pub const CENTI_WATR: Balance = 10 * MILLI_WATR;
pub const WATR: Balance = 100 * CENTI_WATR;

pub const EXISTENTIAL_DEPOSIT: Balance = CENTI_WATR;

const fn deposit(items: u32, bytes: u32) -> Balance {
	(items as Balance * 20 * WATR + (bytes as Balance) * 100 * MILLI_WATR) / 100
}
...
parameter_types! {
	pub const RewardAmount: Balance = 6_000 * MILLI_WATR;
}
...
parameter_types! {
	pub const AssetDeposit: Balance = 100 * WATR; // 100 WATR deposit to create fungible asset class
...
parameter_types! {
	...
	pub const DidDeposit: Balance = 10 * WATR;
}
...
parameter_types! {
	...
	pub BlockGasLimit: U256 = U256::from(NORMAL_DISPATCH_RATIO * MAXIMUM_BLOCK_WEIGHT.ref_time() / WEIGHT_REF_TIME_PER_GAS);
	pub PrecompilesValue: FrontierPrecompiles<Runtime> = FrontierPrecompiles::<_>::new();
	pub WeightPerGas: Weight = Weight::from_parts(WEIGHT_REF_TIME_PER_GAS, 0);
...
construct_runtime!(
	pub struct Runtime
	{
		...
		BlockReward: pallet_block_reward::{Pallet, Call, Storage, Event<T>, Config<T>} = 12,
...
impl_runtime_apis! {
	...
	impl frame_try_runtime::TryRuntime<Block> for Runtime {
		fn on_runtime_upgrade(checks: frame_try_runtime::UpgradeCheckSelect) -> (Weight, Weight) {
			log::info!("try-runtime::on_runtime_upgrade watr-runtime.");
