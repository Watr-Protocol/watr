...
use frame_support::traits::fungibles::roles::Inspect as RoleInspect;
...
pub struct TusdPrecompile<Runtime, Instance: 'static = ()>(
    PhantomData<(Runtime, Instance)>,
);

impl<Runtime, Instance> TusdPrecompile<Runtime, Instance> {
    pub fn new() -> Self {
        Self(PhantomData)
    }
}
...
impl<Runtime, Instance> PrecompileSet for TusdPrecompile<Runtime, Instance>
where
    Instance: 'static,
    ...
    <Runtime as pallet_assets::Config<Instance>>::AssetId: From<u128>,
    ...
    <<Runtime as frame_system::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin: OriginTrait,
{
    fn execute(&self, handle: &mut impl PrecompileHandle) -> Option<EvmResult<PrecompileOutput>> {
        ...
        let asset_id: AssetIdOf<Runtime, Instance> = 2010u128.into();
        let owner = pallet_assets::Pallet::<Runtime, Instance>::issuer(asset_id);

        // only allow asset issuer (i.e., TUSD contract) to call precompile
        if Some(origin.clone()) == owner.clone() {
            log::info!("***** owner {:?}, origin {:?}", owner.clone(), origin.clone());
