...
pub mod pallet {
	...
	pub type CollatorReward<T> = StorageValue<_, BalanceOf<T>, ValueQuery>;
	...
	pub struct GenesisConfig<T: Config> {
		...
		pub collator_reward: BalanceOf<T>,
	}
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			<CollatorReward<T>>::put(&self.collator_reward);
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		/// Get a unique, inaccessible account id from the `RewardId`.
		pub fn reward_id() -> T::AccountId {
			T::RewardId::get().into_account_truncating()
		}
		...
		pallet_authorship::EventHandler<T::AccountId, T::BlockNumber> for Pallet<T>
	{
		fn note_author(author: T::AccountId) {
			let fee_pot = Self::account_id();
			let collator_pot = Self::reward_id();
			// assumes an ED will be sent to pot.
			let fee_reward = T::Currency::free_balance(&fee_pot)
				.checked_sub(&T::Currency::minimum_balance())
				.unwrap_or_else(Zero::zero)
				.div(2u32.into());
			// `reward` is half of pot account minus ED, this should never fail.
			let _success = T::Currency::transfer(&fee_pot, &author, fee_reward, KeepAlive);
			let collator_reward = Self::collator_reward();
			let _collator_success: DispatchResult = match T::Currency::free_balance(&collator_pot)
				.checked_sub(&T::Currency::minimum_balance().add(collator_reward))
			{
				Some(_) =>
			...
			debug_assert!(_collator_success.is_ok());
